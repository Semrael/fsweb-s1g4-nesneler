...
function MenuElemaniOlustur(name, price, categori /*Kodlar buraya*/) {
  const menuElemani = {
    isim: name,
    fiyat: price,
    kategori: categori,
...
console.log("G1b", MenuElemaniOlustur("Karışık Pizaa", 5, "Pizzalar"));
/* Görev 2: 
...
console.log("G2", burger.indirim("öğretmen"));
console.log("G2", burger.indirim("öğrenci"));
console.log("G2", burger.indirim("diğer"));
...
*/
console.log("G3", degerlendirmeler[5].geribildirim);
...
console.log("G4", degerlendirmeler[7].geribildirim);
console.log("yeni", degerlendirmeler);
...
function DegerlendirmeEkle(
  ...
  isim,
  puan,
  geribildirim /*Kodlar buraya */
) {
  const yeniDizi = (dDizisi = {
    isim: isim,
    puan: puan,
    geribildirim: geribildirim,
  });
  degerlendirmeler.push(yeniDizi);
...
function AnahtardanDegerlendirmeAl(dizi, diziIndeksi /*Kodlar buraya*/) {
  const chosenFeedBack = dizi[diziIndeksi];
  ...
  const str1 =
    ...
    chosenFeedBack.geribildirim; /*Kodlar buraya*/

  const str = `${chosenFeedBack.isim} isimli kişi ${chosenFeedBack.puan} puan verdi ve şunları yazdı:${chosenFeedBack.geribildirim} `;
  return str;
}

console.log("G6", AnahtardanDegerlendirmeAl(degerlendirmeler, 0));
/*  Görev 7:  
...
function SonDegerlendirmeyiAl(dizi /*Kodlar buraya*/) {
  /*Kodlar buraya*/
  const sonArray = dizi[dizi.length - 1];
  const donen = `${sonArray.isim} isimli kişi ${sonArray.puan} puan verdi ve şunları yazdı:${sonArray.geribildirim}`;
  return donen;
}
console.log("G7", SonDegerlendirmeyiAl(degerlendirmeler));
/////////////// BONUS  GÖRVLER////////////////////
...
function PuanaGoreDegerlendirmeAl(dizi, score /* Kodlar buraya */) {
  const bosDizi = [];
  for (let i = 0; i < dizi.length; i++) {
    const dArray = dizi[i];
    if (Math.floor(dArray.puan) === score) {
      bosDizi.push(dArray);
    }
  }
...
function UzunDegerlendirmeleriAl(arr /* Kodlar buraya */) {
  const uzunDeger = [];
  for (let i = 0; i < arr.length; i++) {
    let geri = arr[i]["geribildirim"].split(" ").length;
    if (geri > 15) {
...
function arabaYapici(ilkKm /* Kodlar buraya */) {
  const araba = {
    sayac: ilkKm,
    marka: "nehir",
    vites: "Auto",
...
const araba4x4 = new arabaYapici(10);
const araba4x2 = new arabaYapici(0);

console.log("4x4 surus", araba4x4.surus(40));
console.log("4x4 surus", araba4x4.surus(70));
